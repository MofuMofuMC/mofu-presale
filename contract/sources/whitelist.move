module presale::whitelist {
    ...
    /// Get the quantity a user is eligible to mint during whitelist period
    /// Returns 0 if user is not whitelisted or if outside minting time period
    #[view]
    public fun is_user_eligible_for_whitelist_mint(user_addr: address): u64 acquires WhitelistConfig {
        ...
        let is_whitelisted = table::contains(&config.whitelisted_address, user_addr);
        if (is_whitelisted) {
            // Check if current time is within minting period
            if (now >= config.whitelist_presale_start_time
                && now <= config.whitelist_presale_end_time) {
    ...
    public fun test_init_whitelist_config_ok(
    ...
    public fun test_add_duplicate_whitelist(
        sender: &signer, user: &signer
    ) acquires WhitelistConfig {
        init_whitelist_config(sender);
    ...
    public fun test_remove_nonexistent_whitelist(
        sender: &signer, user: &signer
    ) acquires WhitelistConfig {
        init_whitelist_config(sender);
        let user_addr = signer::address_of(user);
    ...
    public fun test_is_user_eligible_for_whitelist_mint_ok(
    ...
    #[test(sender = @presale, user = @0xA)]
    public fun test_decrease_whitelist_mint_amount_ok(
        sender: &signer, user: &signer
    ) acquires WhitelistConfig {
        init_whitelist_config(sender);
        let user_addr = signer::address_of(user);
    ...
    public fun test_decrease_whitelist_mint_amount_insufficient(
        sender: &signer, user: &signer
    ) acquires WhitelistConfig {
        init_whitelist_config(sender);
        let user_addr = signer::address_of(user);
    ...
    #[test(sender = @presale, user = @0xA)]
    #[expected_failure(abort_code = EWHITELIST_NOT_EXISTS, location = presale::whitelist)]
    public fun test_decrease_whitelist_mint_amount_not_whitelisted(
        sender: &signer, user: &signer
    ) acquires WhitelistConfig {
        init_whitelist_config(sender);
